...
def plot_per_episode(
    ...
    def info_reward(step_info_per_episode, step_info_per_eval_episode, window: str = '10s'):
        ...
        reward_roll_mean = pd.concat([train_reward.rolling(window).mean(), test_reward.sort_index().rolling(window).mean()])
        reward_roll_vol = pd.concat([train_reward.rolling(window).std() ** 0.5, test_reward.sort_index().rolling(window).std() ** 0.5])
