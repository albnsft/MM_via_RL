...
class BookImbalance(Feature):
    def __init__(
        ...
        super().__init__("BookImbalance", -1, 1, update_frequency, 0, normalisation_on, max_norm_len)
...
class SellDistance(Feature):
    def __init__(
        ...
        super().__init__(name, 0, 1000000, update_frequency, 0, normalisation_on, max_norm_len)
    ...
    def _update(self, state: State) -> None:
        if state.sell_parameter:
            ...
            self.current_value = mid_price + state.sell_parameter * spread
        else:
            self.current_value = 0
